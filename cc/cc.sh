...
# Toggle OFF : si la fenêtre CC est déjà ouverte, la fermer.
if window_open; then
...
pkill -f "[q]uickshell.*cc/main.qml" >/dev/null 2>&1 || true
...
# Attente de la réponse du serveur (max ~6s — démarrage en ~1s).
server_up=0
